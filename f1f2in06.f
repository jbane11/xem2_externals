...
!--------------------------------------------------------------------
      ...
      REAL*8 W1p,W2P,DW2DPF,WSQP,pf,kf,es,dw2des,W1D,W2D
      real*8 xtmp,f2a,f1a
      real A4, x4, fitemc, emcfac, f2allm, ra,dra
      logical goodfit
      INTEGER ISM
...
C call f2allm for large W

        IF(wsq.gt.6.0) then
           xtmp=qsq/2.0/pm/nu
c           f2a=f2allm(sngl(xtmp),sngl(qsq))
c           call r1998(sngl(xtmp),sngl(qsq),ra,dra,goodfit)
           call ineft(qsq,sqrt(wsq),W1p,W2p,dble(1.0))
c           write(6,*) 'cheesy poofs', qsq,sqrt(wsq),w1p,w2p
           if(Z.lt.0.5) then
              call ineft(qsq,sqrt(wsq),W1D,W2D,dble(2.0))
c              write(6,*) 'bad kitty', qsq,sqrt(wsq),w1d,w2d
              F1a=pm*(2.0*W1D-W1p)
              F2a=nu*(2.0*W2D-W2p)
c              write(6,*) 'danger Will Robinson ',F1a,F2a
           else
              F1a=pm*W1p
              F2a=nu*W2p
           endif

cc           write(6,*) 'cheesy poofs', xtmp,qsq,f2a,ra
           IF(wsq.le.10.0) then
              F2=(10.0-wsq)/4.0*F2 + (wsq-6.0)/4.0* f2a
              F1=(10.0-wsq)/4.0*F1 + (wsq-6.0)/4.0* f1a
c              Rc=(10.0-wsq)/2.0*Rc + (wsq-8.0)/2.0* ra
c              F1 = pm*(F2/nu)*(1.0+nu/qsq)/(1.0+Rc)
c              write(6,*) 'more cheesy poofs',F1,F2
           else
              F2=f2a
              F1=f1a
c              F1= pm*(F2/nu)*(1.0+nu/qsq)/(1.0+ra)
           endif
