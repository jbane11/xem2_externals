...
C Simple model
c	   if(wsq.gt.1.151915) then
cc	      call F1F2IN06(1, 1, Q2, WSQ, F1, F2)
c	      F2=f2allm(sngl(x),sngl(Q2))
c	      call r1998(sngl(x),sngl(Q2),ra,dra,goodfit)
c              F1= m_p*(F2/nu)*(1.0+nu/Q2)/(1.0+ra)
c	   else
c	      F1=0.0
c	      F2=0.0
c	   endif
c	   W1 = F1/m_p
c	   W2 = F2/nu
cc this gives F2 for proton - need to include neutrons, use n/p=1-0.8x
c
c	   W1 = W1*(Z+(A-Z)*(1.0-0.8*x))
c	   W2 = W2*(Z+(A-Z)*(1.0-0.8*x))
c
c	   if(x.lt.0.9) then
c	      emc_corr= emc_func_slac(x,a)
c	   else
c	      emc_corr = emc_func_slac(0.9d0,a)
c	   endif
c
c
c	   W1=W1*emc_corr
